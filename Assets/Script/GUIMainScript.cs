...
public class GUIMainScript : MonoBehaviour {
	...
	void Update () {
		if (Input.GetKeyDown ("q") || (Input.touchCount > 0 && Input.GetTouch(0).phase == TouchPhase.Began && Input.GetTouch(0).position.x > 400)){
        ...
        if ( Input.GetKeyDown( "w" ) || ( Input.touchCount > 0 && Input.GetTouch( 0 ).phase == TouchPhase.Began && Input.GetTouch( 0 ).position.x < 400 ) ) {
